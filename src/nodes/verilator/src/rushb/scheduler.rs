...
use crate::ffi::{
    ...
    verilator_rushb_probes, verilator_rushb_submit, verilator_rushb_take_completed,
};
use crate::Simulator;
use bebop_rushb::{FUNCT7_BARRIER, FUNCT7_FENCE};
...
struct AcceleratorQueue {
    ...
    completion_waits: HashMap<u64, CompletionWait>,
}
...
fn process_accepts(queues: &mut HashMap<u32, AcceleratorQueue>, cycle: u64) -> Result<(), String> {
    ...
    for accelerator_id in accelerator_ids {
        ...
        if matches!(active.request.funct7, FUNCT7_FENCE | FUNCT7_BARRIER) {
            unsafe { verilator_rushb_complete_on_accept(accelerator_id, active.request.command_id) };
        }
        match active.request.wait {
            ...
            WaitMode::Completed => {
                let command_id = active.request.command_id;
                let previous = queue.completion_waits.insert(
                    command_id,
                    CompletionWait {
                ...
                if previous.is_some() {
                    return Err(format!("duplicate rushB completion waiter for command #{command_id}"));
                }
            }
        }
    }
...
fn process_completions(queues: &mut HashMap<u32, AcceleratorQueue>, _cycle: u64) -> Result<(), String> {
    ...
    for accelerator_id in accelerator_ids {
        ...
        loop {
            ...
            if !unsafe { verilator_rushb_take_completed(accelerator_id, &mut command_id) } {
                break;
            }
            let Some(waiting) = queue.completion_waits.remove(&command_id) else {
                continue;
            };
...
fn check_timeouts(queues: &HashMap<u32, AcceleratorQueue>, cycle: u64) -> Result<(), String> {
    for (&accelerator_id, queue) in queues {
        ...
        for waiting in queue.completion_waits.values() {
...
fn is_drained(queues: &HashMap<u32, AcceleratorQueue>) -> bool {
    queues
        .values()
        .all(|queue| queue.queued.is_empty() && queue.active.is_none() && queue.completion_waits.is_empty())
...
fn has_runtime_work(queues: &HashMap<u32, AcceleratorQueue>) -> bool {
    queues
        .values()
        .any(|queue| !queue.queued.is_empty() || queue.active.is_some() || !queue.completion_waits.is_empty())
        || !all_inflight_zero(queues)
}
