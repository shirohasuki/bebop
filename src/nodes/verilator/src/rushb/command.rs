...
use crate::ffi::{
    verilator_host_rush_accepted, verilator_host_rush_complete, verilator_host_rush_complete_on_accept,
    verilator_host_rush_last_ready, verilator_host_rush_last_retired, verilator_host_rush_probes,
    verilator_host_rush_submit,
};
...
fn step_until(state: &mut HostState, accelerator_id: u32, mut done: impl FnMut() -> bool, context: &str) {
    ...
    unsafe {
        panic!(
            ...
            verilator_host_rush_probes(accelerator_id),
            verilator_host_rush_accepted(accelerator_id),
            verilator_host_rush_last_ready(accelerator_id),
            verilator_host_rush_last_retired(accelerator_id),
        );
    }
}
...
pub(crate) fn execute(state: &mut HostState, accelerator_id: u32, xs1: u64, xs2: u64, funct7: u32) {
    ...
    unsafe {
        let accepted_before = verilator_host_rush_accepted(accelerator_id);
        verilator_host_rush_submit(accelerator_id, xs1, xs2, funct7);
        step_until(
            state,
            accelerator_id,
            || verilator_host_rush_accepted(accelerator_id) != accepted_before,
        ...
        if funct7 == FUNCT7_FENCE {
            verilator_host_rush_complete_on_accept(accelerator_id);
            return;
        }
        step_until(
            state,
            accelerator_id,
            || verilator_host_rush_complete(accelerator_id),
